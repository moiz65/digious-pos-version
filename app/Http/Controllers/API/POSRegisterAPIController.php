...
class POSRegisterAPIController extends AppBaseController
{
    ...
    public function closeRegister(Request $request)
    {
        ...
        $register->credit_card = $data['today_sales_credit_card_payment'];
        $register->foodpanda_cc = $data['today_sales_foodpanda_cc_payment'];
        $register->foodpanda_cod = $data['today_sales_foodpanda_cod_payment'];
    ...
    public function getRegisterData($startDate, $endDate)
    {
        ...
        $data['today_sales_credit_card_payment'] = SalesPayment::whereIn('sale_id', $saleIds)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('payment_type', SalesPayment::CREDIT_CARD)
            ->sum('amount');

        $data['today_sales_foodpanda_cc_payment'] = SalesPayment::whereIn('sale_id', $saleIds)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('payment_type', SalesPayment::FOODPANDA_CC)
            ->sum('amount');

        $data['today_sales_foodpanda_cod_payment'] = SalesPayment::whereIn('sale_id', $saleIds)
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('payment_type', SalesPayment::FOODPANDA_COD)
            ->sum('amount');
