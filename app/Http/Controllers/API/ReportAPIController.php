...
class ReportAPIController extends AppBaseController
{
    ...
    public function getTodaySalesOverallReport()
    {
        ...
        $data['today_sales_credit_card_payment'] = SalesPayment::where('payment_date', $today)->where(  
            'payment_type',  
            SalesPayment::CREDIT_CARD  
        )->sum('amount');  
        
        $data['today_sales_foodpanda_cc_payment'] = SalesPayment::where('payment_date', $today)->where(  
            'payment_type',  
            SalesPayment::FOODPANDA_CC  
        )->sum('amount');  
        
        $data['today_sales_foodpanda_cod_payment'] = SalesPayment::where('payment_date', $today)->where(  
            'payment_type',  
            SalesPayment::FOODPANDA_COD  
        )->sum('amount');
